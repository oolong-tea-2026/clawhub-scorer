...
// Clear and auto-detect when user manually picks a local file
document.getElementById("skill-zip").addEventListener("change", async () => {
  // Clear ClawHub slug input
  document.getElementById("skill-slug-download").value = "";
  ...
  // Derive slug from filename (strip .zip)
  const slug = file.name.replace(/\\.zip$/i, "");
  document.getElementById("slug").value = slug;
  document.getElementById("downloads").value = "0";
  document.getElementById("display-name").value = "";

  // Try to extract displayName from SKILL.md frontmatter inside ZIP
  try {
    const buf = await file.arrayBuffer();
    const name = extractNameFromZip(new Uint8Array(buf));
    if (name) document.getElementById("display-name").value = name;
  } catch (e) { /* ignore */ }
});

function extractNameFromZip(zipBytes) {
  // Minimal ZIP scan using Central Directory to find SKILL.md and read frontmatter "name:" field.
  // Central Directory approach is robust against data descriptor (bit 3) ZIPs.
  const decoder = new TextDecoder();
  const data = zipBytes;
  const len = data.length;

  // Find End of Central Directory (EOCD) — scan backwards
  let eocd = -1;
  ...
  let cd = data[eocd + 16] | (data[eocd + 17] << 8) | (data[eocd + 18] << 16) | (data[eocd + 19] << 24);

  for (let i = 0; i < cdCount; i++) {
    if (cd + 46 > len) break;
    if (!(data[cd] === 0x50 && data[cd+1] === 0x4b && data[cd+2] === 0x01 && data[cd+3] === 0x02)) break;

    const method = data[cd + 10] | (data[cd + 11] << 8);
    const compSize = data[cd + 20] | (data[cd + 21] << 8) | (data[cd + 22] << 16) | (data[cd + 23] << 24);
    const nameLen = data[cd + 28] | (data[cd + 29] << 8);
    const extraLen = data[cd + 30] | (data[cd + 31] << 8);
    const commentLen = data[cd + 32] | (data[cd + 33] << 8);
    const localOff = data[cd + 42] | (data[cd + 43] << 8) | (data[cd + 44] << 16) | (data[cd + 45] << 24);

    const fname = decoder.decode(data.slice(cd + 46, cd + 46 + nameLen));
    cd += 46 + nameLen + extraLen + commentLen;

    // Match SKILL.md by basename, only STORED (we can't inflate in this simple scanner)
    if (fname.split("/").pop() === "SKILL.md" && method === 0) {
      const locNameLen = data[localOff + 26] | (data[localOff + 27] << 8);
      const locExtraLen = data[localOff + 28] | (data[localOff + 29] << 8);
      const start = localOff + 30 + locNameLen + locExtraLen;
      const content = decoder.decode(data.slice(start, start + Math.min(compSize, 2000)));
      const m = content.match(/^---[\\s\\S]*?^name:\\s*["']?(.+?)["']?\\s*$/m);
      if (m) return m[1];
    }
  }
  return null;
...
document.getElementById("download-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    // Auto-fill slug — always overwrite
    document.getElementById("slug").value = slug;

    // Clear any previous file input visual state (we're replacing it)
    // and auto-fill display name and downloads from detail API
    document.getElementById("display-name").value = "";
    document.getElementById("downloads").value = "0";
    if (detailResp.ok) {
